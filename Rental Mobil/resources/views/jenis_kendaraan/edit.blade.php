        ...
        <div class="card">
            ...
            <div class="card-body">
                @foreach ($data_jenis_kendaraan as $jenis_kendaraan)
                    <form action="{{ url('jenis_kendaraan/update/' . $jenis_kendaraan->id) }}" method="POST">
                        {{ csrf_field() }}
                        <div class="form-group">
                            <label for="nama">Nama Jenis Kendaraan</label>
                            <input type="text" class="form-control" value="{{ $jenis_kendaraan->nama }}" id="nama" name="nama" required>
                                <br>
